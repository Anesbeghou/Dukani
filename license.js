/**
 * DAKANI LICENSE SYSTEM v3.1
 * - المفتاح مرتبط ببصمة الجهاز رياضياً (أوفلاين كامل)
 * - لا يمكن نقل المفتاح بين الأجهزة
 * - كشف أدوات المطور (F12)
 *
 * ⚠️ جديد v3.1 — تجديد بصمة الترخيص تلقائياً:
 *   بصمة الجهاز المستخدمة في مفتاح الترخيص (DKN-...) تتجدّد تلقائياً
 *   (تصبح قيمة مختلفة كلياً) في حالتين: (1) عند اكتشاف أن المفتاح
 *   المحفوظ منتهي الصلاحية، (2) عند اكتشاف أدوات المطور (F12).
 *   الهدف: منع إعادة تفعيل نفس المفتاح القديم بإعادة إدخاله بعد
 *   انتهائه أو بعد محاولة عبث بالجهاز.
 *
 *   لكن مفتاح المدير (MGR-... في accounts.js) لا يتأثر بهذا التجديد
 *   إطلاقاً، لأنه يُبنى على بصمة "دائمة" منفصلة (getPermanentDeviceId)
 *   لا تتغيّر أبداً. بهذا يبقى دخول المدير شغالاً بمفتاحه القديم حتى
 *   لو انتهى ترخيص التطبيق وتغيّرت بصمة الترخيص المعروضة على الشاشة.
 */
...
const DakaniLicense = (() => {
  ...
  const DEVICE_ID_KEY = 'dakani_dev_id'; // بصمة "الترخيص" — تتجدّد عند انتهاء الترخيص أو كشف أدوات المطور (F12)
  const PERM_ID_KEY = 'dakani_perm_dev_id'; // بصمة "دائمة" ثابتة للأبد — لا تتجدّد أبداً، يُبنى عليها مفتاح المدير في accounts.js
  const FP_SALT_KEY = 'dakani_fp_salt'; // عدّاد يُستخدم فقط عند التجديد لضمان بصمة جديدة مختلفة فعلياً
  ...
  // ─── بصمة الجهاز (6 أحرف) ────────────────────────────────
  // ملاحظة مهمة: بعض القيم (screen.width/height خاصة) تتغيّر عند تدوير
  // الشاشة أو عند إعادة تشغيل الجهاز (Tablet/Phone) قبل استقرار نظام التشغيل،
  // مما كان يجعل الجهاز نفسه يُحسب بمعرّف مختلف في كل مرة ويُعتبر خطأً "جهازاً جديداً".
  // الحل: نحسب البصمة مرة واحدة فقط ونخزّنها محلياً، وبعدها نعيد نفس القيمة المخزّنة
  // دائماً لهذا الجهاز — إلا إذا استُدعي التجديد الصريح (انظر _regenerateFingerprint).
  function _rawFingerprint(salt) {
    const raw = [
      navigator.language       || '',
      navigator.platform       || '',
      ...
      salt || '0' // يُغيَّر فقط عند التجديد المتعمّد، وإلا يبقى ثابتاً
    ].join('|');
  ...
  function _fingerprint() {
    ...
    let salt = '0';
    try { salt = localStorage.getItem(FP_SALT_KEY) || '0'; } catch (e) {}
    const id = _rawFingerprint(salt);

    try { localStorage.setItem(DEVICE_ID_KEY, id); } catch (e) {}
    return id;
  }

  // ─── تجديد بصمة "الترخيص" فقط (وليس البصمة الدائمة) ─────────
  // يُستدعى تلقائياً عند: (1) اكتشاف انتهاء صلاحية المفتاح المحفوظ
  // (2) اكتشاف أدوات المطور (F12). الهدف: أي مفتاح قديم كان صالحاً
  // على هذا الجهاز يصبح فوراً غير متطابق مع البصمة الجديدة، فلا يمكن
  // إعادة استخدامه أو إعادة إدخاله بعد ذلك تحت أي ظرف.
  function _regenerateFingerprint() {
    let salt = 0;
    try { salt = parseInt(localStorage.getItem(FP_SALT_KEY) || '0', 10) || 0; } catch (e) {}
    salt += 1;
    const id = _rawFingerprint(String(salt));
    try {
      localStorage.setItem(FP_SALT_KEY, String(salt));
      localStorage.setItem(DEVICE_ID_KEY, id);
    } catch (e) {}
    return id;
  }

  // ─── بصمة "دائمة" لا تتجدّد أبداً ─────────────────────────────
  // يُبنى عليها مفتاح المدير (accounts.js) خصيصاً حتى يبقى شغالاً
  // حتى بعد تجدّد بصمة الترخيص (انتهاء الاشتراك أو كشف F12). أول مرة
  // تُستدعى فيها بعد هذا التحديث، نعتمد بصمة الترخيص الحالية كقيمة
  // ابتدائية — حتى تبقى مفاتيح المدير الصادرة سابقاً شغالة دون الحاجة
  // لإعادة إصدارها.
  function getPermanentDeviceId() {
    try {
      const perm = localStorage.getItem(PERM_ID_KEY);
      if (perm) return perm;
    } catch (e) {}
    const current = _fingerprint();
    try { localStorage.setItem(PERM_ID_KEY, current); } catch (e) {}
    return current;
  }
  ...
  function verify(key) {
    ...
    if (new Date() > parsed.expiry) {
      // ⚠️ فور اكتشاف انتهاء الصلاحية، نولّد بصمة ترخيص جديدة لهذا الجهاز.
      // من الآن فصاعداً حتى نفس المفتاح المنتهي لن يتطابق أبداً مع الجهاز
      // (لأن بصمته تغيّرت)، فيستحيل تفعيله من جديد بإعادة إدخاله فقط.
      _regenerateFingerprint();
  ...
  function _lockdown() {
    ...
    _regenerateFingerprint(); // بصمة جديدة أيضاً عند كشف أدوات المطور — لإبطال أي مفتاح كان محفوظاً
  ...
  return { gate, activate, verify, info, getDeviceId, getPermanentDeviceId };
